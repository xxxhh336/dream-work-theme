...
5. Normally leave `apps` empty and use registry defaults; add explicit entries only for incompatibility or an application-specific layout.
...
`copy` is optional. Compatibility uses registry defaults plus manifest overrides: an undeclared application uses its `acceptsGenericThemes` value, while `compat` or `layout` is only needed for an explicit exception.

The floating menu no longer contains fixed theme IDs. It shows up to four frequently used preset themes for each application. A new theme appears in the gallery through registry-default compatibility or an explicit manifest override; applying it from Dream Work Theme or selecting it from the floating menu increments usage for that application and can promote it into the quick menu. With no usage history, compatible themes fill the available quick-menu slots.
...
For AstronClaw compatibility, check New Task, existing conversations, My Skills, and the Inspiration Gallery. Generic `[class*="message"]`, `bg-card`, or `backdrop-filter` rules must not create large opaque overlays, and restore tests must cover native light, dark, and system preferences through AstronClaw's settings API.
